...
export class ApiClient {
  ...
  private async ensureValidToken(): Promise<string | null> {
    ...
    const now = Date.now();
    console.log(
      `[ApiClient] Current time: ${now}, lastCheckTime: ${this.lastCheckTime}`
    );
    ...
    console.log(
      "[ApiClient] Got token from authService:",
      token ? "[HIDDEN]" : "null"
    );
    ...
    try {
      ...
      if (isExpired) {
        ...
        try {
          ...
          console.log(
            "[ApiClient] Got new token after refresh:",
            newToken ? "[HIDDEN]" : "null"
          );
