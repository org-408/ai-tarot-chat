...
export async function GET() {
  logWithContext("info", "📍 /api/auth/ticket - チケット発行リクエスト受信", {
  ...
  try {
    ...
    if (!ticket) {
      logWithContext("error", "❌ チケットが取得できませんでした");
      throw new Error("チケットが取得できませんでした");
    }
    logWithContext("info", "✅ チケット発行成功", { token: ticket });

    return Response.json({ ticket });
  } catch (error) {
    logWithContext("error", "❌ チケット発行エラー", { error });
    ...
    logWithContext("error", "❌ チケット発行リクエストで予期せぬエラー", {
      errorMessage,
      status: statusCode,
    });
