...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (!spread) {
      logWithContext("error", `❌ スプレッド(${id})が見つかりません`, { status: 404 });
    ...
    logWithContext("error", `❌ スプレッド(${id})取得エラー`, { error, status: 500 });
    return NextResponse.json(
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    logWithContext("error", `❌ スプレッド(${id})更新エラー`, { error, status: 500 });
    return NextResponse.json(
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    logWithContext("error", `❌ スプレッド(${id})削除エラー`, { error, status: 500 });
    return NextResponse.json(
