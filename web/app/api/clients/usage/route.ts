...
export async function GET(request: NextRequest) {
  ...
  try {
    await logWithContext('info', "ユーザー(client)のユーザー利用状況を取得", { path });
    ...
    await logWithContext('info', "ユーザー利用状況取得完了", { clientId, userStats });
    return NextResponse.json(userStats);
  } catch (error) {
    await logWithContext('error', "ユーザー利用状況取得エラー", { error, clientId });
