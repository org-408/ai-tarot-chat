      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <Row label="本日の占い回数" value={String(client.dailyReadingsCount)} />
            <Row label="本日のパーソナル回数" value={String(client.dailyPersonalCount)} />
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            直近のログ（最新20件）
            <Link
              href={`/admin/logs?q=${encodeURIComponent(client.id)}`}
              className="ml-3 text-xs text-sky-600 hover:underline font-normal"
            >
              全件表示 →
            </Link>
